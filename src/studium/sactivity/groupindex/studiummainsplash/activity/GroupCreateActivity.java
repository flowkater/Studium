...
public class GroupCreateActivity extends SherlockActivity implements
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (group_name_edit_text.length() < 3) {
			Toast.makeText(getApplicationContext(), "그룹 이름은 3자 이상!",
					Toast.LENGTH_LONG).show();
			return true;
		} else if (group_goal_edit_text.length() < 3) {
			Toast.makeText(getApplicationContext(), "목표는 3자 이상!",
					Toast.LENGTH_LONG).show();
			return true;
		} else if (group_location_edit_text.length() < 1) {
			Toast.makeText(getApplicationContext(), "지역은 2자 이상!",
					Toast.LENGTH_LONG).show();
			return true;
		} else {
			showDialog(0);
	...
	protected Dialog onCreateDialog(int id) { // Dialog preference
		...
		case 0: {
			...
			mProgressDialog.setMessage("그룹 ?�성중입?�다...");
